...
#import "FF_DPBH5Manage.h"
#import "AFNetworking.h"
#import "SSZipArchive/SSZipArchive.h"

@implementation FF_DPBH5Manage

+ (instancetype)shareManager
{
    static FF_DPBH5Manage* instance;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        instance = [[FF_DPBH5Manage alloc] init];
    });
    return instance;
}
...
- (void)YFSKingFF_DPBLoadH5PackageWithUrl:(NSString *)url header:(NSDictionary *)header parameters:(id)parameters method:(NSString *)method andPackageName:(NSString *)packageName andVersionCompetetion:(FF_DPBH5ManageH5VersionBlock)result{
    ...
    manager.responseSerializer.acceptableContentTypes = FF_DPBAcceptTypes;
    manager.requestSerializer = [AFJSONRequestSerializer serializer];
    if (header) {
        NSArray* keyArray = header.allKeys;
        for (NSString* key in keyArray) {
            [manager.requestSerializer setValue:[self FF_DPBStringWith:header[key]] forHTTPHeaderField:[self FF_DPBStringWith:key]];
        }
    }
    
    NSString* requestUrl = [[self FF_DPBStringWith:url] stringByAddingPercentEncodingWithAllowedCharacters:[NSCharacterSet URLQueryAllowedCharacterSet]];
        ...
        [manager POST:requestUrl parameters:parameters headers:nil progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
            ...
            if (resdict && resdict[@"data"] && [resdict[@"data"] isKindOfClass:[NSArray class]]) {
                ...
                if (inforArray.count >0) {
                    ...
                    //服务器H5版本号
                    NSString* h5Version = [self FF_DPBStringWith:infoDict[@"h5Version"]];
                    //服务器Zip下载链接
                    NSString* h5Link = [self FF_DPBStringWith:infoDict[@"h5Link"]];
                    //本地H5版本号
                    NSArray* localArr = [[NSUserDefaults standardUserDefaults] objectForKey:[NSString stringWithFormat:@"FF_DPBH5Zip%@",packageName]];
//                    if (self.isLog) {
...
//                        NSString *app_Name = [infoDictionary objectForKey:@"CFBundleDisplayName"];
    ...
    //                        }
                        ...
                        if (localArr == nil && isload) {
                            #pragma mark 需要下载
                            //下载中
                            [[NSUserDefaults standardUserDefaults] setObject:@"0" forKey:[NSString stringWithFormat:@"FF_DPBLoading%@",packageName]];
                            [self FF_DPBDownloadWebZipDataWithUrl:h5Link andPackageName:packageName andCompetetion:^(BOOL loadResult) {
                                if (loadResult) {
                                    //下载成功
                                    [[NSUserDefaults standardUserDefaults] setObject:@"1" forKey:[NSString stringWithFormat:@"FF_DPBLoading%@",packageName]];
                                    [[NSUserDefaults standardUserDefaults] setObject:remoteArray forKey:[NSString stringWithFormat:@"FF_DPBH5Zip%@",packageName]];
                                    if (self.resultBlock && [self.loadPageName isEqualToString:packageName]) {
                                        self.resultBlock(FF_DPBH5LoadSuccess, 1.0);
                                    }
                                }else
                                {
                                    //下载失败
                                    [[NSUserDefaults standardUserDefaults] setObject:@"-1" forKey:[NSString stringWithFormat:@"FF_DPBLoading%@",packageName]];
                                    [[NSUserDefaults standardUserDefaults] setObject:@[] forKey:[NSString stringWithFormat:@"FF_DPBH5Zip%@",packageName]];
                                    if (self.resultBlock && [self.loadPageName isEqualToString:packageName]) {
                                        self.resultBlock(FF_DPBH5LoadFail, 0.0);
                                    }
                                }
...
{
    if ([self loadingState:packageName] == FF_DPBH5LoadSuccess) {
        
        resultBlock(FF_DPBH5LoadSuccess, 1.0);
    }else if([self loadingState:packageName] == FF_DPBH5LoadFail)
    {
        resultBlock(FF_DPBH5LoadFail, 0.0);
    }else
    {
...
{
    NSString* pageState = [[NSUserDefaults standardUserDefaults] objectForKey:[NSString stringWithFormat:@"FF_DPBLoading%@",packageName]];
    if ([[self FF_DPBStringWith:pageState] integerValue] == 1) {
        return FF_DPBH5LoadSuccess;
        
    }else if ([[self FF_DPBStringWith:pageState] integerValue] == -1)
    {
        return FF_DPBH5LoadFail;
    }else
    {
        return FF_DPBH5LoadProcess;
    }
...
- (void)FF_DPBDownloadWebZipDataWithUrl:(NSString* )url andPackageName:(NSString* )packageName andCompetetion:(FF_DPBH5ManageDownloadH5Block)loadResult
{
    ...
    __block NSString *path = [NSString stringWithFormat:@"%@/%@/",NSHomeDirectory(),FF_DPBH5ZipFileCachePath];
    ...
    NSURLSessionDownloadTask *_downloadTask = [manager downloadTaskWithRequest:request progress:^(NSProgress * _Nonnull downloadProgress) {
        ...
        if (self.isLog) {
            NSLog(@"(FF_DPBH5Manage/%@)-H5下载进度==%.2f",packageName,curr/total);
        }
        
        if (self.resultBlock && [self.loadPageName isEqualToString:packageName]) {
            CGFloat progress = curr/total;
            self.resultBlock(FF_DPBH5LoadProcess, progress);
        }
        ...
        if (error) {
            ...
            NSString * tempPath = [NSString stringWithFormat:@"%@/%@/%@",NSHomeDirectory(),FF_DPBH5ZipFileBasePath,packageName];
            @try {
...
//字符串判空处理
- (NSString* )FF_DPBStringWith:(NSString* )string
{
...
- (void)startUnzipH5FileZip:(NSString*)localZipFile FileName:(NSString *)FileName finishBlock:(void(^)(BOOL sucess))finishBlock{
    if([[NSFileManager defaultManager] fileExistsAtPath:localZipFile] && [localZipFile hasSuffix:@"zip"]){
        NSString * newPath = [NSString stringWithFormat:@"%@/%@/%@",NSHomeDirectory(),FF_DPBH5ZipFileBasePath,FileName];
